...
public class AccountService {
    ...
    public Account updateAccountByAttribute(Long accountId, AccountFormDTO updatedAttributes) {
        Optional<Account> optionalAccount = accountRepository.findById(accountId);
    
        if (optionalAccount.isPresent()) {
            Account account = optionalAccount.get();
    
            // Update the specified attributes using AccountFormDTO with null checks
            if (updatedAttributes.getCompanyName() != null) {
                account.setCompanyName(updatedAttributes.getCompanyName());
            }
            if (updatedAttributes.getAddress() != null) {
                account.setAddress(updatedAttributes.getAddress());
            }
            if (updatedAttributes.getState() != null) {
                account.setState(updatedAttributes.getState());
            }
            if (updatedAttributes.getRegion() != null) {
                account.setRegion(updatedAttributes.getRegion());
            }
            if (updatedAttributes.getWebsite() != null) {
                account.setWebsite(updatedAttributes.getWebsite());
            }
            if (updatedAttributes.getTypeOfBusiness() != null) {
                account.setTypeOfBusiness(updatedAttributes.getTypeOfBusiness());
            }
            if (updatedAttributes.getPhone() != null) {
                account.setPhone(updatedAttributes.getPhone());
            }
            if (updatedAttributes.getEmail() != null) {
                account.setEmail(updatedAttributes.getEmail());
            }
            if (updatedAttributes.getTypeOfAccount() != null) {
                account.setTypeOfAccount(updatedAttributes.getTypeOfAccount());
            }
            if (updatedAttributes.getActive() != null) {
                account.setActive(updatedAttributes.getActive());
            }
