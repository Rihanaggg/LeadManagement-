...
public class ContactService {
    ...
    public Contact updateContactByAttribute(Long contactId, ContactFormDTO updatedAttributes) {
        Optional<Contact> optionalContact = contactRepository.findById(contactId);
    
        if (optionalContact.isPresent()) {
            Contact contact = optionalContact.get();
    
            // Update the specified attributes using ContactFormDTO with null checks
            if (updatedAttributes.getName() != null) {
                contact.setName(updatedAttributes.getName());
            }
            if (updatedAttributes.getEmail() != null) {
                contact.setEmail(updatedAttributes.getEmail());
            }
            if (updatedAttributes.getPhone() != null) {
                contact.setPhone(updatedAttributes.getPhone());
            }
            if (updatedAttributes.getJobTitle() != null) {
                contact.setJobTitle(updatedAttributes.getJobTitle());
            }
            if (updatedAttributes.getAccountName() != null) {
                contact.setAccountName(updatedAttributes.getAccountName());
            }
            if (updatedAttributes.getLeadSource() != null) {
                contact.setLeadSource(updatedAttributes.getLeadSource());
            }
